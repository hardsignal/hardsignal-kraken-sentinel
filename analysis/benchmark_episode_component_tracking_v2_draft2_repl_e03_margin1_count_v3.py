...
PATTERNS=TESTS+['test_episode_component_tracking_v2_draft2_repl_e02_margin1.py','test_episode_component_tracking_v2_draft2_repl_e03_queries.py','test_episode_component_tracking_v2_draft2_repl_e03_margin1_count.py','test_episode_component_tracking_v2_draft2_repl_e03_margin1_count_v2.py','test_episode_component_tracking_v2_draft2_repl_e03_margin1_count_v3.py']
...
def sources():
    return [ROOT/p for p in staged.SOURCE_VERSIONS]+[Path(new.__file__).resolve(),Path(new.fallback.__file__).resolve(),Path(new.fallback.dyadic.__file__).resolve(),Path(new.fallback.dyadic.previous.__file__).resolve(),Path(queries.__file__).resolve(),Path(__file__).resolve(),
...
def child(path,ordering):
    ...
    histogram=new.fallback.histogram
    ...
    try:
        with staged.deadline(LIMITS['max_seconds']),patch.object(new.fallback,'histogram',traced_hist):
