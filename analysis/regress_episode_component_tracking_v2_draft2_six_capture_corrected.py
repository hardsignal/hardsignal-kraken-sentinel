"""Corrected bounded regression; immutable historical inventory and scientific parity."""
...
import episode_component_tracking_v2_draft2_repl_e03_margin1_count_v3 as counter
import episode_component_tracking_v2_draft2_repl_e05_margin1_count as cutoff
import episode_component_tracking_v2_draft2_repl_e03_exact_queries as queries
import regress_episode_component_tracking_v2_draft2_six_capture as historical
import episode_component_tracking_v2_draft2_cardinality_sidecar as sc

ROOT=Path(__file__).resolve().parents[1]
PLAN=ROOT/'results/episode-component-tracking-v2-draft2-six-capture-corrected-plan.json'
PLAN_SHA256='04739e46633155bcbdf567ae04f00c74cd196f8e9ec8d282163fc989a7284006'
OLD=ROOT/'results/episode-component-tracking-v2-draft2-limited-regression'
TESTS=historical.TESTS+['test_episode_component_tracking_v2_draft2_'+name+'.py' for name in
    ('repl_e02_margin1','repl_e03_queries','repl_e03_margin1_count',
     'repl_e03_margin1_count_v2','repl_e03_margin1_count_v3','repl_e05_margin1_count',
     'six_capture_corrected')]
BASE=ROOT/'results/episode-component-tracking-v2-draft2-six-capture-regression'
QUERY_BASE=ROOT/'results/episode-component-tracking-v2-draft2-repl-e03-query-investigation'
BASELINE=ROOT/'results/episode-component-tracking-v2-draft2-six-capture-corrected-baseline.json'
SCIENTIFIC_FIELDS=('representation','family_complete_as_predicate','association','metrics',
                   'query_operations','query_calls','computational_limits')


def count_exact(family,budget):
    # Compose committed general implementations; no identity or expected-count dispatch.
    with patch.object(cutoff.previous,'compose',cutoff.compose):
        result=counter.count(family,budget)
    result['dispatch']=dict(certificate_backend=counter.VERSION,
        fallback_backend=counter.fallback.VERSION,composition_backend=cutoff.VERSION)
    return result


def original_parity(result,selected,arm,family_path):
    directory=BASE/selected['label']/arm
    old=json.loads((directory/'result.json').read_text())
    old_case=json.loads((directory/'case.json').read_text())
    if old_case['passed']:
        for field in SCIENTIFIC_FIELDS+('states','cardinality'):
            if result.get(field)!=old.get(field):raise ValueError('Original passing payload changed: '+field)
    else:
        if old['states']['exact_queries']['state']=='EXACT':
            for field in SCIENTIFIC_FIELDS:
                if result.get(field)!=old.get(field):raise ValueError('Completed query payload changed: '+field)
        else:
            milestone=json.loads((QUERY_BASE/f'{arm}-forward.result.json').read_text())
            for field in SCIENTIFIC_FIELDS:
                if result.get(field)!=milestone.get(field):raise ValueError('Query milestone payload changed: '+field)
            for field,value in old['query_operations'].items():
                if value['state']=='EXACT' and result['query_operations'][field]!=value:
                    raise ValueError('Previously exact operation changed: '+field)
        if old['cardinality']['state']=='EXACT' and result['cardinality']!=old['cardinality']:
            raise ValueError('Previously exact cardinality changed')
    if family_path and sc.sha(family_path)!=sc.sha(directory/'family.json'):
        raise ValueError('Original family bytes changed')
    changes={k:dict(before=old['states'][k],after=v) for k,v in result['states'].items() if v!=old['states'][k]}
    return dict(state='PASS',previously_passed=old_case['passed'],state_changes=changes,
                old_result_sha256=sc.sha(directory/'result.json'),
                scientific_payload_sha256=staged.digest({k:result.get(k) for k in SCIENTIFIC_FIELDS}))
...
def read_plan():
    ...
    previous_plan=historical.read_plan()
    for key in ('selected','captures','arms','scientific_semantics','count_limits','query_limits',
                'family_construction_seconds','worker_timeout_seconds','worker_peak_rss_limit_kib'):
        if p[key]!=previous_plan[key]:raise ValueError('Frozen plan mismatch: '+key)
    return p


def sources():
    return sorted(set([*ROOT.glob('analysis/*episode_component_tracking*.py'),
        Path(__file__).resolve(),BASELINE,PLAN]+[ROOT/'tests'/p for p in TESTS]))
...
def execute(manifest,selected,arm,directory,reload=False):
    ...
    if reload:
        ...
        with construction_limits(plan['family_construction_seconds']) as rebuild:
            queries.reproduce(ROOT,saved,fs)
    ...
    if arm!='connected':
        ...
        try:
            with staged.deadline(plan['count_limits']['max_seconds']):counted=count_exact(family,budget)
    ...
    with construction_limits(plan['family_construction_seconds']) as rebuild:
        result,query_stats=queries.integrate(ROOT,req,fs,manifest['git_context'],queries.engine_reference(ROOT,'forward'))
    ...
    prior_parity=original_parity(result,selected,arm,family_path)
    ...
    return dict(states=result['states'],projection=projection,limited_parity=parity,
        original_parity=prior_parity,query_engine_statistics=query_stats,
...
def passed(row):
    ...
    if a.get('original_parity',{}).get('state')!='PASS':return False
...
def run_case(manifest_path,selected,arm,plan):
    ...
    row['counting']=None if count is None else dict(state=count['count_state'],value_decimal=count['exact_count_decimal'],reason=count.get('failure'),statistics=count['statistics'],packed_polynomial_bytes=count['packed_polynomial_bytes'],method=count['method'],certificate=count.get('certificate'))
    ...
    row['record']=dict(capture=selected['capture'],episode=selected['episode'],arm=arm,
        ...
        exact_backend=count['method'] if count else 'HISTORICAL_CONNECTED',
        certificate=count.get('certificate') if count else None,
        exact_dispatch=count.get('details',{}).get('dispatch') if count else None,
...
def mandatory_controls(rows):
    expected={('C1-E02','margin-0'):'1',('C1-E02','margin-0.25'):'32743',
        ('C1-E02','margin-1'):'5788836',('C6-E02','margin-1'):'217075',
    ...
    found={(r['label'],r['arm']):r for r in rows};checks={}
    for key,value in expected.items():
        r=found.get(key,{})
        checks['/'.join(key)]=r.get('passed',False) and (r.get('counting') or {}).get('value_decimal')==value
    for arm in ('margin-0.25','margin-1'):
        a=found.get(('C6-E03',arm),{}).get('audit',{})
        checks['C6-E03/'+arm+'/queries']=(a.get('query_calls')==1012 and
            a.get('status')=='MULTIPLE_PERSISTENT_TRACKS' and a.get('primary') is None and
            a.get('original_parity',{}).get('state')=='PASS')
...
def run(output):
    ...
    plan=read_plan()
    for selected in plan['selected']:limited.saved_episode(plan,selected)
    ...
    manifest=dict(format='KRAKEN_DRAFT2_SIX_CAPTURE_CORRECTED_BATCH_V1',plan_sha256=PLAN_SHA256,git_context=context,
    ...
    mandatory=mandatory_controls(rows)
    assert subprocess.check_output(['git','rev-parse','HEAD']).decode().strip()==plan['git_commit']
    decision='READY_FOR_DRAFT2_FULL_MATRIX_REVIEW' if len(rows)==plan['case_count'] and all(r['passed'] for r in rows) and mandatory['passed'] else 'NOT_READY'
    report=dict(format='KRAKEN_DRAFT2_SIX_CAPTURE_CORRECTED_REGRESSION_V1',decision=decision,plan_sha256=PLAN_SHA256,
