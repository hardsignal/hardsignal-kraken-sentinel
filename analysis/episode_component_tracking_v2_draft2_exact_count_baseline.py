...
def frontier_histogram(rows,cap,budget,label):
    ...
    for i,row in enumerate(rows):
        ...
        for key,count in states.items():
            ...
            for c,w in row:
                ...
                dest=(((used | bit)&suffix[i+1])<<width) | (cost+w)
...
def matching_histogram(family,weights,optimum,cutoff,budget,label):
    ...
    for i in small:
        ...
        minimum=min(w for c,w in options); base+=minimum
        rows.append([(c,w-minimum) for c,w in options])
    cap=optimum+cutoff-base
    if cap<0: return {}
    # Drop only edges individually exceeding the TOTAL remaining budget.
    rows=[[(c,w) for c,w in row if w<=cap] for row in rows]
    # Factor the remaining row-column incidence graph, including singleton rows.
    ...
    for number,indices in enumerate(components):
        ...
        divisor=math.gcd(*(w for row in component for c,w in row)) or 1
    ...
    result={base+k-optimum:v for k,v in histogram.items() if base+k<=optimum+cutoff}
    if any(k<0 for k in result): raise CountUnresolved('Counted a cover below the certified optimum')
    return result
