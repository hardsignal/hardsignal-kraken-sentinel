...
import diagnose_episode_component_tracking_v2_draft2_e02_margin1 as diagnostic

ROOT=Path(__file__).resolve().parents[1]
OUTPUT=diagnostic.OUTPUT
LIMITS=diagnostic.LIMITS
REGRESSION=ROOT/'results/episode-component-tracking-v2-draft2-limited-regression'
TARGET=REGRESSION/'TPMS004-ordinary/margin-1'
PATTERNS=['test_episode_component_tracking_v2_draft2_'+s+'.py' for s in
          ('e02_margin1_count','staged','exact_solver','exact_count','column_count','cardinality_sidecar','margin1_investigation')]
...
def integration(row,context):
    ...
    episode=next(e for e in raw['episodes'] if e['capture']=='PIPELINE-TPMS-004-20260917-233423' and e['episode']==2)
...
def run():
    ...
    for case in regression['cases']:
        if case['arm']=='connected' or case['audit']['cardinality']['state']!='EXACT':continue
        label=case['label']+'/'+case['arm']
        request=json.loads((REGRESSION/label/'request.json').read_text())
        expected=case['audit']['cardinality']['value_decimal']
    ...
    report=dict(format='KRAKEN_DRAFT2_E02_MARGIN1_COUNT_INVESTIGATION_V1',git_context=context,
        ...
        decision='E02_MARGIN1_EXACT_SOLVED' if solved else 'E02_MARGIN1_STILL_UNRESOLVED',
