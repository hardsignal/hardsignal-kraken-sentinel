...
import diagnose_episode_component_tracking_v2_draft2_repl_e02_margin1 as diagnostic

ROOT=Path(__file__).resolve().parents[1]
OUTPUT=diagnostic.OUTPUT
LIMITS=diagnostic.LIMITS
REGRESSION=ROOT/'results/episode-component-tracking-v2-draft2-six-capture-regression'
TARGET=REGRESSION/'C6-E02/margin-1'
from regress_episode_component_tracking_v2_draft2_six_capture import TESTS
PATTERNS=TESTS+['test_episode_component_tracking_v2_draft2_repl_e02_margin1.py']
...
def sources():
    return [ROOT/p for p in staged.SOURCE_VERSIONS]+[Path(new.__file__).resolve(),Path(new.previous.__file__).resolve(),Path(__file__).resolve(),
...
def child(path,ordering):
    ...
    try:
        with staged.deadline(LIMITS['max_seconds']),patch.object(old,'ieee_histogram',traced_hist):
...
def integration(row,context):
    ...
    episode=next(e for e in raw['episodes'] if e['capture']=='BETWEEN-DEVICE-REPL-V1-20260919-024341' and e['episode']==2)
...
def run():
    ...
    report=dict(format='KRAKEN_DRAFT2_REPL_E02_MARGIN1_COUNT_INVESTIGATION_V1',git_context=context,
        ...
        decision='REPL_E02_MARGIN1_EXACT_SOLVED' if solved else 'REPL_E02_MARGIN1_STILL_UNRESOLVED',
