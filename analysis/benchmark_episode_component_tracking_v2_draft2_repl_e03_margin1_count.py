...
PATTERNS=TESTS+['test_episode_component_tracking_v2_draft2_repl_e02_margin1.py','test_episode_component_tracking_v2_draft2_repl_e03_queries.py','test_episode_component_tracking_v2_draft2_repl_e03_margin1_count.py']
...
def run():
    ...
    if not (OUTPUT/'diagnostic-pivot.json').exists():raise ValueError('Reproduce/diagnose baseline first')
    ...
    report=dict(format='KRAKEN_DRAFT2_REPL_E03_MARGIN1_COUNT_INVESTIGATION_V1',git_context=context,
        ...
        diagnostic_sha256=sc.sha(OUTPUT/'diagnostic-pivot.json'))
