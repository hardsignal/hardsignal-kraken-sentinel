"""Counting-only controls, then E02 target, then target-only staged validation."""
import io
import json
...
import diagnose_episode_component_tracking_v2_draft2_repl_e05_margin1_count as diagnostic

ROOT=Path(__file__).resolve().parents[1]
OUTPUT=diagnostic.OUTPUT
LIMITS=diagnostic.LIMITS
REGRESSION=ROOT/'results/episode-component-tracking-v2-draft2-limited-regression'
TARGET=diagnostic.BASE
PATTERNS=['test_episode_component_tracking_v2_draft2_'+s+'.py' for s in
          ('e02_margin1_count','staged','exact_solver','exact_count','column_count','cardinality_sidecar','margin1_investigation','repl_e02_margin1','repl_e03_queries','six_capture','repl_e05_margin1_count')]
...
def sources():
    return [ROOT/p for p in staged.SOURCE_VERSIONS]+[Path(new.__file__).resolve(),Path(__file__).resolve(),
            Path(diagnostic.__file__).resolve(),Path(new.previous.__file__).resolve(),Path(new.dyadic.__file__).resolve()]+[ROOT/'tests'/p for p in PATTERNS]
...
def child(path,ordering):
    ...
    try:
        with staged.deadline(LIMITS['max_seconds']):
...
def integration(row,context):
    ...
    old_result=json.loads((TARGET/'result.json').read_text())
    ...
    result=staged.integrate(ROOT,request,episode['components'],context)
    ...
    start=time.perf_counter();reloaded=staged.reproduce(ROOT,json.loads((OUTPUT/'target-staged-result.json').read_text()),episode['components'])
    ...
    return dict(state='PASS',family_unchanged=True,exact_queries_unchanged=True,association_unchanged=True,
        ...
        old_result_sha256=sc.sha(TARGET/'result.json'))


def run():
    if (OUTPUT/'benchmark.json').exists():raise ValueError('Benchmark output must be NEW')
    if not (OUTPUT/'diagnostic.json').exists():raise ValueError('Reproduce/diagnose baseline first')
    ...
    controls=[]
    six=ROOT/'results/episode-component-tracking-v2-draft2-six-capture-regression'
    selected=[]
    for path in sorted(six.glob('*/margin-*/count-outcome.json')):
        recorded=json.loads(path.read_text())
        if recorded['count_state']=='EXACT':
            selected.append((path.parent/'family.json',recorded['exact_count_decimal']))
    # Mandatory E08 controls reference existing certified families via requests.
    for arm,expected in [('margin-0','2'),('margin-0.25','17067800502243'),
                         ('margin-1','378038954451765697490')]:
        request=json.loads((REGRESSION/'TPMS008-control'/arm/'request.json').read_text())
        selected.append((ROOT/request['family']['path'],expected))
    selected.append((path.parent/'family.json',recorded['exact_count_decimal']))
    selected.append((six/'C6-E02/margin-1/family.json','217075'))
    for family_path,expected in selected:
        label=str(family_path.parent.relative_to(ROOT))
        for ordering in ('forward','reverse'):
            result=launch(str(family_path.relative_to(ROOT)),ordering)
    ...
    report=dict(format='KRAKEN_DRAFT2_REPL_E05_MARGIN1_COUNT_INVESTIGATION_V1',git_context=context,
        ...
        decision='REPL_E05_MARGIN1_EXACT_SOLVED' if solved else 'REPL_E05_MARGIN1_STILL_UNRESOLVED',
        ...
        full_28_case_regression_rerun=False,rf_capture=False,spectrum_detection=False,
        diagnostic_sha256=sc.sha(OUTPUT/'diagnostic.json'))
