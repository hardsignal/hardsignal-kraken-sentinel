    ...
    if rep == COMPACT:
        ...
        try:
            with deadline(limits['max_seconds']):
                witness = budget.call(family.find)
                if witness is None or not budget.call(family.contains,witness):
                ...
                for factor in family.factors:
                    for k in range(len(factor.solver.paths)):
                        ...
                        if budget.call(family.possible,path):
                            possible.add(path)
                out['query_operations'][operation] = state('EXACT')
                operation = 'invariance'
                # An invariant path must belong to the verified witness.
                invariant = {tuple(p) for p in witness if budget.call(family.invariant,p)}
                out['query_operations'][operation] = state('EXACT')
                operation = 'ambiguity'
                classification = budget.call(family.classify)
                out['query_operations'][operation] = state('EXACT')
